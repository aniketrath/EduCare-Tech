...
import { createSkill } from '../../api/SkillsHelper';
...
export default function CreateSkill() {
	...
	const submit = () => {
		...
		dispatch(showAlert('Creating skill... Please wait'));
		createSkill(data.title, data.file).then((res) => {
				...
				<Box>
					<Box className='py-6 px-[4%] center'>
						<Box horizontal className='center'>
							<Text className='text-xl font-medium capitalize text-dark'>Create Skill</Text>
						</Box>
