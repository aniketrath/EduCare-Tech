...
const CreateAuthCookie = async (res: Response, _user: IUser) => {
...
const CreateAdminAuthCookie = async (res: Response, _user: IUser) => {
	if (!res || !_user) return new Error('Parameters cannot be empty');

	const user = await Admin.findById(_user._id).select('refreshTokens');
	if (!user) return new Error('User not found');
...
export { CreateAuthCookie, CreateAdminAuthCookie, CreateCookie };
